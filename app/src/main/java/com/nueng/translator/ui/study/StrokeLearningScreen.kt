...
fun StrokeLearningContent(modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        Text("Basic Strokes (基本笔画)", fontSize = 18.sp, fontWeight = FontWeight.Bold,
            color = accentPurple, modifier = Modifier.padding(vertical = 4.dp))
        Text("The 6 fundamental strokes", fontSize = 12.sp,
            color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(8.dp))
        PinyinData.basicStrokes.forEach { StrokeCard(it, accentPurple); Spacer(Modifier.height(6.dp)) }

        Spacer(Modifier.height(12.dp))
        Text("Turning Strokes (转折笔画)", fontSize = 18.sp, fontWeight = FontWeight.Bold,
            color = accentTeal, modifier = Modifier.padding(vertical = 4.dp))
        Spacer(Modifier.height(8.dp))
        PinyinData.turningStrokes.forEach { StrokeCard(it, accentTeal); Spacer(Modifier.height(6.dp)) }

        Spacer(Modifier.height(12.dp))
        Text("Writing Rules (书写规则)", fontSize = 18.sp, fontWeight = FontWeight.Bold,
            color = accentAmber, modifier = Modifier.padding(vertical = 4.dp))
        Spacer(Modifier.height(8.dp))
        PinyinData.writingRules.forEachIndexed { index, rule ->
            Card(
                ...
                Row(Modifier.padding(14.dp), verticalAlignment = Alignment.Top) {
                    Text((index + 1).toString() + ".", fontSize = 14.sp,
    ...
    Card(
        ...
        Row(Modifier.padding(16.dp).fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            if (stroke.unicode.isNotBlank()) {
                Text(stroke.unicode, fontSize = 40.sp, fontWeight = FontWeight.Bold,
            ...
            Column(Modifier.weight(1f)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Text(stroke.chinese, fontSize = 20.sp, fontWeight = FontWeight.Bold,
                        color = CardTextPrimary)
                    Spacer(Modifier.width(8.dp))
                    Text(stroke.pinyin, fontSize = 14.sp,
                        color = CardTextHint, fontStyle = FontStyle.Italic)
                }
                if (stroke.name.isNotBlank())
                    Text(stroke.name, fontSize = 13.sp, color = accentColor.copy(0.8f))
                if (stroke.description.isNotBlank()) {
                    Spacer(Modifier.height(4.dp))
                    Text(stroke.description, fontSize = 12.sp,
                        color = CardTextHint, lineHeight = 16.sp)
                }
            }
            IconButton(onClick = { TtsHelper.speak(stroke.chinese, "zh") }) {
                Icon(Icons.AutoMirrored.Filled.VolumeUp, "Speak",
                    tint     = accentColor,
