...
fun PinyinLearningContent(modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        Text("Simple Vowels (单韵母)", fontSize = 18.sp, fontWeight = FontWeight.Bold,
            color = accentBlue, modifier = Modifier.padding(vertical = 4.dp))
        Text("Tap each tone to hear its sound", fontSize = 12.sp,
            color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(8.dp))
        PinyinData.simpleVowels.forEach { item -> VowelCard(item, accentBlue); Spacer(Modifier.height(8.dp)) }

        Spacer(Modifier.height(12.dp))
        Text("Compound Vowels (复韵母)", fontSize = 18.sp, fontWeight = FontWeight.Bold,
            color = accentGreen, modifier = Modifier.padding(vertical = 4.dp))
        Text("Tap each tone to hear — " + PinyinData.compoundVowels.size + " combinations",
            fontSize = 12.sp, color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(8.dp))
        PinyinData.compoundVowels.forEach { item -> VowelCard(item, accentGreen); Spacer(Modifier.height(8.dp)) }

        Spacer(Modifier.height(12.dp))
        Text("Consonants (声母)", fontSize = 18.sp, fontWeight = FontWeight.Bold,
            color = accentOrange, modifier = Modifier.padding(vertical = 4.dp))
        Text("23 initial consonants — tap to hear", fontSize = 12.sp,
            color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(8.dp))
...
private fun VowelCard(item: PinyinItem, accentColor: Color) {
    Card(
        modifier = Modifier.fillMaxWidth()
            .border(1.dp, accentColor.copy(0.4f), RoundedCornerShape(12.dp)),
        ...
        Column(Modifier.padding(16.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically,
                ...
                Row {
                    IconButton(onClick = { TtsHelper.speak(item.example.ifBlank { item.pinyin }, "zh") }) {
                        Icon(Icons.AutoMirrored.Filled.VolumeUp, "Play",
                            tint = accentColor, modifier = Modifier.size(28.dp))
                    }
                    IconButton(onClick = { TtsHelper.speakSlow(item.example.ifBlank { item.pinyin }, "zh") }) {
                        Icon(Icons.Default.SlowMotionVideo, "Slow",
                            tint = accentColor.copy(0.6f), modifier = Modifier.size(24.dp))
                    }
                }
            }
                ...
                FlowRow(horizontalArrangement = Arrangement.spacedBy(10.dp),
                    ...
                    item.tones.forEachIndexed { index, tone ->
                        ...
                        Column(
                            ...
                            Text(tone, fontSize = 22.sp, fontWeight = FontWeight.Medium,
            ...
            if (item.example.isNotBlank()) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text(item.examplePinyin, fontSize = 13.sp,
                        color = CardTextHint, fontStyle = FontStyle.Italic)
                }
            }
        }
    }
...
private fun CompactPinyinChip(item: PinyinItem, accentColor: Color) {
    Card(
        modifier = Modifier
            .clickable { TtsHelper.speak(item.example.ifBlank { item.pinyin }, "zh") }
        ...
        Column(Modifier.padding(horizontal = 14.dp, vertical = 10.dp),
            ...
            if (item.example.isNotBlank())
                Text(item.example, fontSize = 13.sp, color = CardTextPrimary.copy(0.7f))
        }
    }
}
