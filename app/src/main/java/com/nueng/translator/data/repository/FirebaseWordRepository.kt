...
class FirebaseWordRepository @Inject constructor(
    ...
    fun pushWordToFirebase(word: LanguageWord): String {
        ...
        wordsRef.child(key).setValue(buildMap(word))
        return key
    }

    fun updateWordOnFirebase(word: LanguageWord) {
        if (word.firebaseKey.isBlank()) return
        wordsRef.child(word.firebaseKey).setValue(buildMap(word))
    }
    ...
    private fun buildMap(word: LanguageWord): Map<String, Any> = mapOf(
        "word"                       to word.word,
        "wordType"                   to word.wordType,
        "pinyin"                     to word.pinyin,
        "langCode"                   to word.langCode,
    ...
    fun startRealtimeSync() {
        ...
        // PERF: keep language_words synced to disk cache — served offline after first load
